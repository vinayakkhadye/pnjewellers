...
class ModelToolExportImport extends Model {
	...
	protected function uploadCategories( &$reader, $incremental, &$available_category_ids=array() ) {
		...
		//custom_code_vinayak
		$upload_dir_path = '';
		if(isset($this->request->post['upload_dir_path'])){
			$upload_dir_path = $this->request->post['upload_dir_path'];
		}
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$category['image'] = $this->uploadImage($category_id, $upload_dir_path, $image_name, false, 'category');
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$product['image'] = $this->uploadImage($product_id, $upload_dir_path, $image_name, false, 'product');
	...
	// custom_code_vinayak
	public function uploadImage($product_id, $upload_dir_path = '', $image_name, $additional = False, $image_type) {
		$upload_image_file = $upload_dir_path. "/". $image_name;
		$ext = strtolower(pathinfo($upload_image_file, PATHINFO_EXTENSION));
		switch($image_type){
			case 'product':
				$prefix = 'pr_';
			break;
		...
		$folder = $prefix . $product_id % 10;
		$catalog_dir = 'catalog/'.$folder;
		$directory = DIR_IMAGE . $catalog_dir;

		$destination_file = $directory . "/".$product_id . ( $additional ? '_' . $additional : '' ) . "." . $ext;
		$catalog_dir_file =  $catalog_dir . "/".$product_id . ( $additional ? '_' . $additional : '' ) . "." . $ext;
		if(!is_dir($directory)) {
			mkdir($directory);
		}

		if(is_file($upload_image_file)) {
			if(copy($upload_image_file, $destination_file)) {
				return $catalog_dir_file;
			}
		}else if (is_file(DIR_IMAGE . $image_name)){
			return $image_name;
		}
		return NULL;
	}
	...
	protected function uploadAdditionalImages( &$reader, $incremental, &$available_product_ids ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$image['image_name'] = $this->uploadImage($product_id, $upload_dir_path, $image_name, $i, 'additional_product');
			if ($exist_sort_order) {
