...
// vinayak_custom_code	
	public function uploadExcel($upload_dir, $file) {
		$this->load->language('extension/export_import');
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('extension/export_import');

		$incremental = ($this->request->post['incremental']) ? true : false;
		$this->request->post['upload_dir_path'] = $upload_dir; 
...
// vinayak_custom_code
	public function uploadCustom() {
		...
		// Accept ZIP file and extract images out of it and save to /categroy/demo
		// then countinue processging excel file
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateUploadForm())) {
		...
		}elseif (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateUploadZip())) {
			$zip_file = DIR_UPLOAD . 'tmp-' . $this->request->files['upload']['name'];
			move_uploaded_file($this->request->files['upload']['tmp_name'], $zip_file);
			
			// Unzip file
			$zip = new ZipArchive();
			if ($zip->open($zip_file)) {
				$upload_dir_path = DIR_UPLOAD . pathinfo($this->request->files['upload']['name'],PATHINFO_FILENAME) ;
				$zip->extractTo($upload_dir_path . "/");
				$zip->close();
				unlink($zip_file);
			}
			...
			$it = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($upload_dir_path));
			$it->rewind();
			while($it->valid()) {
				if (!$it->isDot()) {
					$file = $it->key();
					...
					if (($ext == 'xls') || ($ext == 'xlsx') || ($ext == 'ods')) {
        				$excelPath = pathinfo($file, PATHINFO_DIRNAME);
						$isExcelUploaded = $this->uploadExcel($excelPath, $file);
						break;
					}
				}
				$it->next();
	...
	// vinayak_custom_code
		$data['import'] = $this->url->link('extension/export_import/uploadCustom', 'user_token=' . $this->session->data['user_token'], $this->ssl);
	...
	// custom_vinayak_code
	protected function validateUploadZip() {
		$this->error = array();		
