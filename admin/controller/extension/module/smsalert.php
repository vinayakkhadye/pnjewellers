...
class ControllerExtensionModuleSmsAlert extends Controller {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$exist_template = $this->model_extension_module_smsalert->getTemplateByType($this->request->post['type']);
			if(!empty($exist_template))
			{
				$this->session->data['success'] = 'Template already exists. Please edit template - '.$exist_template['name'];
	...
	protected function getList() {
		...
		//Added on 06-05-2019 
		$data['smsalert_country']=($this->config->get('smsalert_country')) ? $this->config->get('smsalert_country') : '91';	
		...
		$data['selected_payment_methods'] =explode(',',$this->config->get('smsalert_payment_method'));
		$data['smsalert_mobile_pattern'] =$this->config->get('smsalert_mobile_pattern');
		
		
		/*/-get payment methods*/
		
		$data['sms_credit'] = $this->model_extension_module_smsalert->getSMSCredits();
       ...
       //Closed Added on 06-05-2019
		...
		foreach ($templates as $template) {
			...
			if ($type) {
				...
				$type = $this->language->get('Return ' . $template['type']);
			}
	...
	public function install() {
		...
		$this->db->query("INSERT IGNORE INTO `" . DB_PREFIX . "sms_template` (`sms_template_id`, `type`, `store_id`, `name`, `bcc`) VALUES
			(1, 'register', 0, 'Account Registration (Default)', ''),
			(2, 'affiliate', 0, 'Affiliate Registration (Default)', ''),
			(3, '1', 0, 'Order Confirmation (Default)', ''),
			...
			(11, 'otp_for_register', 0, 'OTP For Register', ''),
			(12, 'otp_for_login', 0, 'OTP For Login', ''),
			(13, 'otp_for_checkout', 0, 'OTP For Checkout', '');
		");
		...
		$this->db->query("INSERT IGNORE INTO `" . DB_PREFIX . "sms_template_message` (`sms_template_id`, `language_id`, `message`) VALUES
			...
			(11, 1, 'Your verification code is [otp]'),
			(12, 1, 'Your verification code is [otp]'),
			(13, 1, 'Your verification code is [otp]');
		");
		...
		$this->model_setting_event->addEvent('module_smsalert', 'catalog/controller/account/login/before', 'extension/module/smsalert/eventPreControllerAccountLogin');
		...
		$this->model_setting_event->addEvent('module_smsalert', 'catalog/model/account/customer/addCustomer/before', 'extension/module/smsalert/eventPreModelAccountCustomerAdd');
		...
		$this->model_setting_event->addEvent('module_smsalert', 'catalog/model/checkout/order/addOrderHistory/after', 'extension/module/smsalert/eventPostModelCheckoutOrderAddOrderHistory');
